...
impl DiffEngine {
    ...
    pub fn to_comparable_yaml(resource: &Value) -> Result<String> {
        ...
        crate::yaml::serialize_yaml_value(&normalized)
    ...
    pub async fn diff_yaml_with_server(
        ...
        let desired_yaml = crate::yaml::serialize_yaml_value(&desired_normalized)
            .map_err(|e| crate::NylError::Config(format!("Failed to serialize resource to YAML: {}", e)))?;
        let live_yaml = crate::yaml::serialize_yaml_value(&live_normalized)
            .map_err(|e| crate::NylError::Config(format!("Failed to serialize resource to YAML: {}", e)))?;
