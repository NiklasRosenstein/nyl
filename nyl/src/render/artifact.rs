...
impl DirectoryVendorWriter {
    ...
    pub fn sync(
        ...
        atomic_replace(
            ...
            format!("# Generated by nyl vendor sync; do not edit.\n{serialized}").as_bytes(),
        )?;
        Ok(VendorSyncResult {
    ...
    fn attributes(&self, lock: &VendorLock) -> String {
        ...
        rules.sort();
        let mut lines = vec!["# Generated by nyl vendor sync; do not edit.".to_owned()];
        lines.extend(rules);
    ...
    pub fn lookup(&self, request: &ArtifactRequest) -> Result<Option<ResolvedArtifact>> {
        ...
        if let Some(vendor) = self.vendor.as_ref().filter(|_| !population_refresh) {
            ...
            if (self.vendor_mode == VendorMode::Required || vendor_check) && !population {
                return Err(NylError::config(format!(
                    "Remote artifact {} is not present in the required vendor lock; run 'nyl vendor sync'",
                    request.display()
                )));
            }
        }
        if vendor_check {
            return Err(NylError::config(format!(
                "Remote artifact {} is not present in the vendor lock; run 'nyl vendor sync'",
                request.display()
            )));
        }
